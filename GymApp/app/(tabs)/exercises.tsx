...
import {
  ...
  Modal, ScrollView, Image, ImageBackground, Linking, Platform,
} from 'react-native';
...
// Icono emoji por grupo muscular
...
const ExerciseCard = ({ item, isAdmin, onView, onEdit, onDelete }: ExerciseCardProps) => {
  ...
  const icon = GROUP_ICONS[item.muscle_group] ?? GROUP_ICONS.default;
  ...
  return (
    ...
    >
      ...
      <View style={s.cardInfo}>
        ...
        {/* Badge grupo muscular con icono */}
...
const ExerciseDetailModal = ({ exercise, onClose }: DetailModalProps) => {
  ...
  const icon = GROUP_ICONS[exercise.muscle_group] ?? GROUP_ICONS.default;
          ...
          <View style={s.detailContent}>
            ...
            {exercise.video_url && (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <View style={s.videoButtonIcon}>
                  <Text style={{ color: G.white, fontSize: 16 }}>▶</Text>
                </View>
...
export default function ExercisesScreen() {
  ...
  useEffect(() => {
    load('Todos');
    getRole().then(role => setIsAdmin(role?.toLowerCase().trim() === 'admin'));
  }, []);
  ...
  const handleDelete = async (exercise: Exercise) => {
    ...
    if (Platform.OS === 'web') {
      if (window.confirm(`¿Seguro que quieres eliminar "${exercise.name}"?`)) await borrar();
    } else {
  ...
  if (isAdmin === null) {
    return <View style={s.center}><ActivityIndicator size="large" color={G.primary} /></View>;
  }

  return (
    <SafeAreaView style={s.container}>

      {/* ── HEADER — fondo fondoInicio.png ── */}
      <ImageBackground
      ...
      >
        ...
        <View style={s.headerRow}>
          ...
          <View style={s.headerActions}>
            {/* Icono mancuerna en círculo verde */}
      ...
      >
        {MUSCLE_GROUPS.map(g => {
          ...
          const icon = g !== 'Todos' ? GROUP_ICONS[g] ?? '' : null;
          return (
            <TouchableOpacity
              key={g}
            ...
            >
              ...
              <Text style={[s.chipText, active && s.chipTextActive]}>{g}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
          ...
          <View style={s.modalCard}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={s.modalHeader}>
                ...
                <TouchableOpacity
                  onPress={() => { setCreateVisible(false); setForm(EMPTY_FORM); }}
                  hitSlop={{ top:10, bottom:10, left:10, right:10 }}
                >
                  <Text style={{ fontSize: 20, color: G.textHint }}>✕</Text>
                </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  headerOverlay: {
    ...
    backgroundColor: 'rgba(242, 245, 238, 0.72)',
  },
  ...
  addBtnText: { fontSize: 12, color: G.white, fontWeight: '700', letterSpacing: 0.5 },
  ...
  // ── Chips ──
  chipsContainer: { flexGrow: 0, flexShrink: 0, marginTop: 4 },
  chipsRow: {
  ...
  chipText: { fontSize: 12, color: G.textSub, fontWeight: '600' },
  ...
  cardDesc: { fontSize: 13, color: G.textSub, lineHeight: 18 },
  ...
  arrowCircle: {
    ...
    borderRadius: 15,
    borderWidth: 1,
    borderColor: G.primaryLt,
    backgroundColor: G.primaryLt,
    alignItems: 'center',
    justifyContent: 'center',
  },
  arrowText: { fontSize: 18, color: G.primary, lineHeight: 22 },
  deleteBtn: { padding: 6 },
  ...
  detailTopBar: {
    ...
    borderBottomWidth: 0.5, borderBottomColor: G.border, backgroundColor: G.white,
  },
  ...
  detailName: { fontSize: 30, fontWeight: '800', color: G.textMain, letterSpacing: -0.5, lineHeight: 34, marginBottom: 20 },
  detailCloseBtn: {
  ...
  detailNoDesc: {
    backgroundColor: G.bgPage, borderRadius: 10, padding: 14,
    borderWidth: 0.5, borderColor: G.border, marginBottom: 8,
  },
  detailNoDescTxt: { fontSize: 13, color: G.textHint, textAlign: 'center' },
  videoButton: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    backgroundColor: G.primary, borderRadius: 14, padding: 16, marginTop: 24,
  },
  videoButtonIcon: {
    width: 34, height: 34, borderRadius: 17,
    backgroundColor: G.primaryDark, justifyContent: 'center', alignItems: 'center',
  },
  videoButtonText: { fontSize: 15, color: G.white, flex: 1 },
  detailCloseFull: {
    marginTop: 16, paddingVertical: 14, borderRadius: 12,
    borderWidth: 0.5, borderColor: G.border, alignItems: 'center',
  },
  ...
  modalTitle: { fontSize: 22, fontWeight: '700', color: G.textMain },
  inputLabel: { fontSize: 10, color: G.textHint, letterSpacing: 1, marginBottom: 8, marginTop: 2, fontWeight: '700' },
  input: {
    backgroundColor: G.bgPage, borderWidth: 0.5, borderColor: G.border,
    borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12,
    fontSize: 14, color: G.textMain, marginBottom: 16,
  },
  ...
  btnSaveText: { color: G.white, fontSize: 14, fontWeight: '700', letterSpacing: 0.5 },
});